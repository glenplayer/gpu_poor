...
function getGradOptMemory(
	...
	if (full === "lora_trn" && opt === "adam_opt" && quant === "no_quant") {
		return parsedConfig.num_layers * 8 * parsedConfig.hiddenDim * 2 * 4 * 3 * 2;
	}
	...
	if (parsedJSONData == null) {
		if (jsonUploadedData != null) {
			...
			if (!isNumberOrFloat(modelSize)) {
				...
				setErrorMessage(
					"Huggingface model id not available, enter model size(>0) or upload config"
				);
				openModal();
				return null;
			}
...
function App() {
	...
	const [contextLen, setContextLen] = useState("200");

	const [promptLen, setPromptLen] = useState("10000");
	...
	const [showTableComputeDisclaimer, setShowTableComputeDisclaimer] =
		useState("");
	const [showTableComputeSmallInfo, setShowTableComputeSmallInfo] = useState(0);
	...
	// set the value of the input field to the value of the item when a suggestion is clicked
	const [selections, setSelections] = useState({
		dropdownTrnOrNot: "inf_ggml",
		dropdownFullOrNot: "full_trn",
		dropdownOpt: "adam_opt",
		dropdownQuant: "ggml_QK4_K_S",
		dropdownGPU: "RTX-4090-24gb",
		dropdownCPU: "3600x",
		dropdownDDR: "ddr4",
		isGPUorCPU: "usingGPU",
		isGradCheckPoint: "no",
	...
	function token_per_second_logic_CPU(
		...
		// );
		let totalFlopsToken =
			2 * memoryTransfer + 2 * totalLen * hiddenDim * 2 * numLayers * 2 * 2;
		...
		const jsonComputeReturnData = {
			"Token/s": Math.round(token_per_s) >= 1 ? Math.round(token_per_s) : "< 1",
	...
	function token_per_second_logic_Train(
		...
		const jsonComputeReturnData = {
			"ms per iteration(forward + backward)": timeIfFlops_in_ms.toFixed(2),
			"memory or compute bound?": memoryOrCompute,
		};
	...
	function token_per_second_logic_GPU(
		...
		let theoryTimePrompt =
			...
			2 * numLayers * hiddenDim * hiddenDim * 2 * promptLen;
		theoryTimePrompt = batchSize * theoryTimePrompt;
		let theoryTimePrompt_in_ms = theoryTimePrompt / (tera * gpu_compute * 0.85);
	...
	async function handleClick() {
		...
		let numGPUsINeed = Math.ceil(
			out["Total"] / (1024 * gpuJSONData[selections.dropdownGPU]["memory"])
		);
		...
		if (modelName && responseCacheKeys) {
			if (modelName.length > 1) {
				...
				if (modelName.includes(" ")) {
					let splitName = modelName.split(" ");
					filtered = responseCacheKeys.filter(
						(item) => item.includes(splitName[0]) && item.includes(splitName[1])
					);
				} else {
					filtered = responseCacheKeys.filter((item) =>
						item.includes(modelName)
					);
				}
						...
						<div>
							...
							<div className="border border-gray-400 p-2 rounded-lg inline-block hover:border-black mt-2">
								<div className="pb-2 flex flex-row">
									...
									{showTrainLenInfo && (
										<div className="text-xs ml-2 text-blue-700">
											For training, set tokens to generate as 1
										</div>
									)}
								</div>
								...
								<div className="flex pb-2 pt-1">
									...
									<div>
										<label className="font-poppins text-sm pr-2">Quant?</label>
										<select
											className="font-poppins text-sm border border-gray-500"
											name="dropdownQuant"
											value={selections.dropdownQuant}
											onChange={handleChangeSelection}
										>
											...
											<option value="ggml_QK4_K_M">GGML QK4_K_M</option>
											<option value="ggml_QK4_K_S">GGML QK4_K_S</option>

											<option value="ggml_QK5_0">GGML QK5_0</option>
											<option value="ggml_QK5_1">GGML QK5_1</option>
											<option value="ggml_QK5_K_M">GGML QK5_K_M</option>
								...
								<div className="flex pt-1">
									<div>
										<label className="font-poppins text-sm">Prompt len? </label>
										<TextInput
									...
									<div className="pl-4">
										<label className="font-poppins text-sm">Batch-size? </label>
										<TextInput
									...
									<div className="flex pt-2 flex-row">
										<div>
											...
											<select
												className="font-poppins text-sm border border-gray-500"
												name="dropdownGPU"
												value={selections.dropdownGPU}
												onChange={handleChangeSelection}
											>
												{Object.keys(gpuJSONData).map((item) => (
									...
									<div className="pt-2 flex flex-row">
										<div className="">
											...
											<select
												...
												onChange={handleChangeSelection}
											>
												<option value="3600x">AMD 3600XT</option>
												<option value="7950x">AMD 7950x</option>
												<option value="12700H">i7-12700H</option>
												<option value="13900K">i9-13900K</option>
												<option value="13700K">i7-13700K</option>
												<option value="9900K">i9 9900K</option>
												<option value="5900X">AMD 5900X</option>
												<option value="5600X">AMD 5600X</option>
												<option value="3990X">AMD 3990X</option>
											</select>
										</div>
										{/* <div className="pl-5">
										...
										<div className="pl-5">
											<label className="font-poppins text-sm pr-2">RAM? </label>
											<select
												className="font-poppins text-sm border border-gray-500"
												name="dropdownDDR"
												onChange={handleChangeSelection}
											>
												{showDDR[0] === 1 && <option value="ddr4">DDR4</option>}
												{showDDR[1] === 1 && <option value="ddr5">DDR5</option>}
											</select>
										</div>
										<div>
											<button
							...
							{showTable && (
								<>
									<div className="text-sm font-poppins font-bold">
										Memory Requirement: {/* {memReqHardwareName} */}
									</div>
									<table className="min-w-1/2 bg-white border-collapse border-2 border-black font-poppins">
										<tbody>
											...
											{/* Name-Value pairs */}
											{Object.entries(breakDownMemoryJson).map(
												([key, value], index) => {
													if (key === "Total") {
							...
							{showTableCompute && (
								<div>
									...
									<table className="min-w-1/2 bg-white border-collapse border-2 border-black font-poppins">
										<tbody>
											{/* Name-Value pairs */}
											{Object.entries(jsonDataCompute).map(
												([key, value], index) => {
													if (key === "Total") {
														return null; // Skip this iteration and return nothing
													}
									...
									{showTableComputeSmallInfo == 1 && (
										<div className="text-xs font-poppins text-blue-700">
											Check above to see how {computedTokenPerSecond} token/s
											looks like
										</div>
									)}
									{showTableComputeSmallInfo == 2 && (
										<div className="text-xs font-poppins text-blue-700">
											For train, generate length = 1. Since training is next
											token pred. e.g. if u train on 500 len sequence then put
											500 in prompt len.
										</div>
									)}
								</div>
								...
								<>
									...
									>
										<tbody>
											{/* Total row */}
											{/* Name-Value pairs */}
											{Object.entries(gpuJsonDataForTable).map(
												([key, value], index) => {
													return (
														<tr
															className={`${
																index % 2 === 0 ? "bg-teal-100" : "bg-teal-50"
															}`}
															key={index}
														>
															<td className="py-1 px-2 text-sm border">
								...
								<>
									...
									>
										<tbody>
											{/* Total row */}
											{/* Name-Value pairs */}
											{Object.entries(cpuJsonDataForTable).map(
												([key, value], index) => {
													return (
														<tr
															className={`${
				...
				<div>
					...
					{faqOpen && (
						<ul className="list-disc pl-5 text-xs font-mono">
							<li>
								These are APPORXIMATE values. They can vary by +-15% depending
								on your CPU, GPU, cuda version, llama.cpp version, model etc.
							</li>
							{/* <li>
							...
							<li>
								CPU inference is only compatible with GGML. You can't use CPU
								with HF/vLLM
							</li>
